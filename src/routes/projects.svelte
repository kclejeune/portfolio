<script lang="ts">
	...
	async function getPinnedRepos(query: string): Promise<Repository[]> {
		...
		return fetch(API_URL, {
			...
			.then((res: PinnedRepoResponse) => res.itemShowcase.edges ?? res.itemShowcase.nodes ?? [])
...
<Page id="projects" title="Projects" backgroundClass={primaryBackground}>
	...
	{:then repos}
		<div class="container grid max-w-screen-xl gap-4 mx-auto md:grid-cols-1 lg:grid-cols-2">
			{#each repos as repo}
				<Card
